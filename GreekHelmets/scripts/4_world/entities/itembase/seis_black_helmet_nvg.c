class seis_black_helmet_nvg : HelmetBase
{
	void seis_black_helmet_nvg()
	{
	...
	{
		// Makes sure helmet doesn't glow when it's attached on head, or it's inside cargo.
		...
		else
		{
			...
			if ( id == id_2) // Helmet is attached on head
				do_glow = false;
			
			if ( id_cargo != -1 ) // Helmet is in cargo
				do_glow = false;
		}
